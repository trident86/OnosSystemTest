...
#            main.generalAttemptsNum = int( main.params[ 'RETRY' ][ 'generalAttempts' ] )
            ...
            main.retrysleep = int( main.params[ 'RSLEEP' ] )
            main.osUtils = imp.load_source( wrapperFile2,
                                           main.dependencyPath +
                                           wrapperFile2 +
                                           ".py" )
            print str(wrapperFile2) + ", " + str(main.dependencyPath)
    ...
    def CASE201( self, main ):
        ...
        stepResult = main.TRUE if resp.status_code == 201 else main.FALSE
        """
        getS1 = utilities.retry( f=main.Cluster.active( 0 ).REST.getNetCfg,
                        retValue=False,
                        kwargs={ "subjectClass": "devices",
                        ...
                        sleep=main.retrysleep )
        onosCfg = pprint( getS1 )
        main.log.info("onos: {}".format( getS1 ) )
        main.log.info("onos: {}".format( onosCfg ) )
        """
        main.sona_network = stepResult
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully populated hostsData",
                                 onfail="Failed to populate hostsData" )
    ...
    def CASE211( self, main ):
        ...
        if main.sona_network == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
	...
	"""
        ...
        main.openstack_network = stepResult
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully created openstack network",
                                 onfail="Failed to create openstack network" )
    ...
    def CASE212( self, main ):
        ...
        if main.openstack_network == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Update openstack network" )
        main.caseExplanation = "Update openstack network for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."

        main.step( "Update openstack network" )
        ...
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully updated openstack network",
                                 onfail="Failed to update openstack network" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE213( self, main ):
        ...
        if main.openstack_network == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Remove openstack network" )
        main.caseExplanation = "Remove openstack network for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully removed openstack network",
                                 onfail="Failed to remove openstack network" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE221( self, main ):
        ...
        main.caseExplanation = "Create openstack subnet for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        stepResult = main.TRUE if result == True else main.FALSE
        main.openstack_subnet = stepResult
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully created openstack subnet",
                                 onfail="Failed to create openstack subnet" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE222( self, main ):
        ...
        if main.openstack_subnet == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Update openstack subnet" )
        main.caseExplanation = "Update openstack subnet for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check openstack modified subnet info
        """

        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully updated openstack subnet",
                                 onfail="Failed to update openstack subnet" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE223( self, main ):
        ...
        if main.openstack_subnet == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Remove openstack subnet" )
        main.caseExplanation = "Remove openstack subnet for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        utilities.assert_equals( expect=main.TRUE,
                                 ...
                                 onfail="Failed to remove openstack subnet" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE231( self, main ):
        ...
        if main.openstack_network == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Create openstack port" )
        main.caseExplanation = "Create openstack port for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check openstack port exists & host detected
        """

        main.openstack_port = stepResult
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully created openstack port",
                                 onfail="Failed to create openstack port" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE232( self, main ):
        ...
        if main.openstack_port == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Update openstack port" )
        main.caseExplanation = "Update openstack port for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check openstack modified port
        """

        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully updated openstack port",
                                 onfail="Failed to update openstack port" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE233( self, main ):
        ...
        if main.openstack_port == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Remove openstack port" )
        main.caseExplanation = "Remove openstack port for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        utilities.assert_equals( expect=main.TRUE,
                                 ...
                                 onfail="Failed to remove openstack port" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )

    def CASE241( self, main ):
        """
        ...
        if main.openstack_port == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Create openstack floatingip" )
        main.caseExplanation = "Create openstack floatingip for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        main.openstack_floatingip = stepResult
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully created openstack floatingip",
                                 onfail="Failed to create openstack floatingip" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )

    def CASE242( self, main ):
        """
        Update openstack floatingip
        """
        import time

        if main.openstack_floatingip == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Update openstack floatingip" )
        main.caseExplanation = "Update openstack floatingip for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check modified host IP
        """

        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully updated openstack floatingip",
                                 onfail="Failed to update openstack floatingip" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE243( self, main ):
        ...
        if main.openstack_floatingip == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Remove openstack floatingip" )
        main.caseExplanation = "Remove openstack floatingip for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check host IP removed
        """

        main.openstack_floatingip = main.FALSE if stepResult == main.TRUE else main.FALSE
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully removed openstack floatingip",
                                 onfail="Failed to remove openstack floatingip" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )

    def CASE251( self, main ):
        """
        ...
        if main.openstack_network == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Create openstack security_group_rule" )
        main.caseExplanation = "Create openstack security_group_rule for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check openstack security_group_rule exists
        """

        main.openstack_security_group_rule = stepResult
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully created openstack security_group_rule",
                                 onfail="Failed to create openstack security_group_rule" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
    ...
    def CASE253( self, main ):
        ...
        if main.openstack_security_group_rule == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Remove openstack security_group_rule" )
        main.caseExplanation = "Remove openstack security_group_rule for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check openstack security_group_rule not exists
        """

        main.openstack_security_group_rule = main.FALSE if stepResult == main.TRUE else main.FALSE
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully removed openstack security_group_rule",
                                 onfail="Failed to remove openstack security_group_rule" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )

    def CASE261( self, main ):
        """
        ...
        """
        TODO: Check openstack security_group exists & host ping with icmp rule
        """

        main.openstack_security_group = stepResult
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully created openstack security_group",
                                 onfail="Failed to create openstack security_group" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )

    def CASE262( self, main ):
        """
        Update openstack security_group
        """
        import time

        if main.openstack_security_group == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Update openstack security_group" )
        main.caseExplanation = "Update openstack security_group for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check openstack security_group info & host ping with icmp rule
        """

        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully updated openstack security_group",
                                 onfail="Failed to update openstack security_group" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )

    def CASE263( self, main ):
        """
        ...
        if main.openstack_security_group == main.FALSE:
            main.log.error( "Test components did not start correctly, skipping further tests" )
            main.skipCase()
        main.case( "Remove openstack security_group" )
        main.caseExplanation = "Remove openstack security_group for devices" +\
                               " not discovered yet. One device is allowed" +\
                               ", the other disallowed."
        ...
        """
        TODO: Check openstack security_group not exists & host ping with icmp rule
        """

        main.openstack_security_group = main.FALSE if stepResult == main.TRUE else main.FALSE
        utilities.assert_equals( expect=main.TRUE,
                                 actual=stepResult,
                                 onpass="Successfully removed openstack security_group",
                                 onfail="Failed to remove openstack security_group" )
        main.log.info( "Sleeping {} seconds".format( main.testSleep ) )
        time.sleep( main.testSleep )
